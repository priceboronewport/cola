...
func (fv *FileVault) Import(filename string, path string, timestamp time.Time) (file_id int, err error) {
	...
	if err == nil && hash_exists {
		err = errors.New("Exists")
	}
	return
}

func (fv *FileVault) ListHash(hash string) (file_ids []int, filenames []string, err error) {
	query := "select f.file_id, concat(path, name) as filename from files f inner join hashes using(hash_id) where hash=? order by 2, 1 desc"
	var rows *sql.Rows
	rows, err = fv.db.Query(query, hash)
	if err == nil {
		defer rows.Close()
		var file_id int
		var filename string
